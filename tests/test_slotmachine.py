...
async def test_handle_win(mocker):
    mock_add_win = mocker.patch("src.slotmachine.add_win", new_callable=mocker.AsyncMock)
...
async def test_handle_loss(mocker):
    mock_add_loss = mocker.patch("src.slotmachine.add_loss", new_callable=mocker.AsyncMock)
